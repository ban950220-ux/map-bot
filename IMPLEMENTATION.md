...
주소 또는 사용자 승인 GPS 좌표 → 검색어/반경의 후보 15~30곳 → 중복·반경 밖 후보 제거 → 4개씩 자동차 경로 계산 → 성공한 DestinationCandidate 배열 → 독립적인 시간/도로거리 정렬 또는 정규화 ETA 80%·거리 20% 추천 점수 → Top N 리스트 및 A/B/C 지도 마커. 선택한 후보의 경로를 강조합니다. 반경은 1/3/5/10/20/50/100/150/200km이며, 결과가 부족하면 사용자가 허용한 경우 단계적으로 최대 200km까지 확장합니다.
