...
const bindings = live ? readMapsCredentials() : { NAVER_MAPS_CLIENT_ID: "fixture-id", NAVER_MAPS_CLIENT_SECRET: "fixture-secret", KAKAO_REST_API_KEY: "fixture-kakao", GOOGLE_PLACES_API_KEY: "fixture-google" };
...
const helpers = await import("data:text/javascript;base64," + Buffer.from(await bundle(`export * from "./services/maps/ranking"; export * from "./services/maps/cache"; export * from "./services/maps/schema"; export * from "./services/maps/searchIntent"; export * from "./services/maps/googlePlaceMatching"; export * from "./lib/nearby-client";`)).toString("base64"));
const script = await bundle(`
 ...
 export default { async fetch(request) {
   ...
   try {
     ...
     if(input.action === "parking") return Response.json(await enrichGoogleStoreParking(input.candidates,undefined,input.timeoutMs));
...
let active = 0, peak = 0, callCount = 0, failureId = 0, quota = false, googleCalls = 0, googleActive = 0, googlePeak = 0, googleFailure = false, googleDelay = 0;
const worker = new Miniflare({ modules: true, script, compatibilityDate: "2026-05-15", compatibilityFlags: ["nodejs_compat"], bindings, cf: false,
  ...(live ? {} : { outboundService: async request => {
    ...
    if(url.hostname === "places.googleapis.com") {
      googleCalls++; googleActive++; googlePeak=Math.max(googlePeak,googleActive);
      try {
        assert.equal(url.pathname,"/v1/places:searchText"); assert.equal(request.method,"POST");
        assert.equal(request.headers.get("x-goog-api-key"),"fixture-google");
        assert.equal(request.headers.get("x-goog-fieldmask"),"places.id,places.displayName,places.formattedAddress,places.location,places.parkingOptions,places.attributions");
        const body=await request.json(); assert.equal(body.pageSize,3); assert.equal(body.languageCode,"ko"); assert.equal(body.regionCode,"KR"); assert.equal(body.locationBias.circle.radius,200);
        if(googleDelay) await new Promise(resolve=>setTimeout(resolve,googleDelay));
        if(googleFailure) return WorkerResponse.json({error:{message:"fixture failure"}},{status:503});
        if(body.textQuery.includes("없는 장소")) return WorkerResponse.json({places:[]});
        const match=body.textQuery.match(/테스트 매장 (\d+)/); const index=Number(match?.[1]||1);
        const candidate=docs[index-1]||docs[0];
        const parkingOptions=index===1?{freeParkingLot:true}:index===2?{freeParkingLot:true,valetParking:true}:undefined;
...
try {
  ...
  if(!live) {
    const google=await invoke({action:"parking",candidates:time.search.candidates.slice(0,3)});
    assert.equal(google.attempted,3); assert.equal(google.matched,3); assert.equal(google.available,2); assert.equal(google.failed,0);
    assert.equal(google.enrichments[0].storeParking.status,"available"); assert.deepEqual(google.enrichments[0].storeParking.types,["무료 주차장"]);
    assert.deepEqual(google.enrichments[1].storeParking.types,["무료 주차장","발렛"]);
    assert.equal(google.enrichments[2].storeParking.status,"unknown"); assert.equal(google.enrichments[2].storeParking.source,"google-places");
    assert.equal(parkingIntent.candidates[0].nearbyParking.status,"found"); assert.equal(google.enrichments[0].storeParking.status,"available","Store and nearby parking stay separate");
    const identity={id:"kakao:700",name:"테스트 매장",address:"경기도 이천시 테스트로 1",latitude:37.27,longitude:127.4};
    const wrongName={id:"wrong",displayName:{text:"전혀 다른 식당"},formattedAddress:"대한민국 경기도 이천시 테스트로 1",location:{latitude:37.27001,longitude:127.40001}};
    const farPlace={id:"far",displayName:{text:"테스트 매장"},formattedAddress:"대한민국 경기도 이천시 테스트로 1",location:{latitude:37.272,longitude:127.4}};
    const wrongAddress={id:"wrong-address",displayName:{text:"테스트 매장"},formattedAddress:"대한민국 서울특별시 강남구 테헤란로 100",location:{latitude:37.27001,longitude:127.40001}};
    assert.equal(helpers.selectGoogleMatch(identity,[wrongName]),undefined,"Nearby place with another name must be rejected");
    assert.equal(helpers.selectGoogleMatch(identity,[farPlace]),undefined,"Distant place must be rejected");
    assert.equal(helpers.selectGoogleMatch(identity,[wrongAddress]),undefined,"Same-name place with another address must be rejected");
    assert.equal(helpers.parkingFromGoogle({...wrongName,displayName:{text:"테스트 매장"}}).status,"unknown","Missing parkingOptions is unknown");
    assert.equal(helpers.parkingFromGoogle({...wrongName,displayName:{text:"테스트 매장"},parkingOptions:{freeParkingLot:false,paidParkingLot:false}}).status,"unknown","False-only parkingOptions is not unavailable");
    const stripped=helpers.withoutStoredGoogleParking({...time.search.candidates[0],storeParking:{status:"available",source:"google-places",types:["무료 주차장"]}});
    assert.equal(stripped.storeParking.status,"unknown"); assert.equal(stripped.storeParking.source,"unknown"); assert.equal(stripped.storeParking.types,undefined,"Google Places content must not enter session storage");
    const noResult=await invoke({action:"parking",candidates:[{...identity,id:"kakao:701",name:"없는 장소"}]});
    assert.equal(noResult.enrichments[0].storeParking.status,"unknown"); assert.equal(noResult.matched,0);
    googleFailure=true; const failedGoogle=await invoke({action:"parking",candidates:[time.ranked[0]]}); googleFailure=false;
    assert.equal(failedGoogle.failed,1); assert.equal(failedGoogle.enrichments[0].storeParking.status,"unknown");
